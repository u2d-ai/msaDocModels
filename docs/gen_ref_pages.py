        ...
        if req_txt and len(req_txt) > 0:
            ...
            for line in req_txt.splitlines():
                ...
                if line == "":
                    ...
                    parts_front = str(line).strip()
                    parts_front = parts_front.replace(" = ", "=").replace('"', "").replace("^", "").replace("~", "")
                    package: str = (
...
def generate_code_reference_documentation(
    ...
    source_path: str = "msaDocModels",
    source_file_type_filter: str = "*.py",
    requirement_file: str = "pyproject.toml",
    md_file_type: str = ".md",
            ...
            with mkdocs_gen_files.open(req_md_file, "w") as fd:
                ...
                for line in req_txt.splitlines():
                    ...
                    if line == "":
                        ...
                        parts_front = str(line).strip()
                        parts_front = (
                            parts_front.replace(" = ", "=").replace('"', "").replace("^", "").replace("~", "")
                        )
                        package: str = str(parts_front).split("=")[0]
                        version: str = ""
                        version_link: str = ""
                        package = package.lower()
